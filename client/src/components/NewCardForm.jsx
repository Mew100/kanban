...
function NewCardForm({ columnId, onCardCreated, dark }) {
  ...
  return (
    <form onSubmit={handleSubmit} style={{ marginTop: '1rem' }}>
      <input type="text" placeholder="Card title" value={title}
        onChange={e => setTitle(e.target.value)} autoFocus style={inputStyle} />
      ...
      {!showAddLabel ? (
        <button type="button" onClick={() => setShowAddLabel(true)} style={{
          fontSize: '0.75rem', color: dark ? '#94a3b8' : '#888',
          background: 'none', border: 'none', cursor: 'pointer', marginBottom: '0.5rem', padding: 0
        }}>
          + Add custom label
        </button>
      ) : (
        <div style={{ display: 'flex', gap: '0.4rem', marginBottom: '0.5rem', alignItems: 'center' }}>
          <input type="text" placeholder="Label name" value={newLabelName}
            ...
            style={{ flex: 1, padding: '0.3rem', borderRadius: '4px', border: dark ? '1px solid rgba(255,255,255,0.15)' : '1px solid #ccc', background: dark ? '#2e2e42' : 'white', color: dark ? '#f1f5f9' : '#1e293b', fontSize: '0.85rem' }}
          />
          ...
          />
          <button type="button" onClick={handleAddLabel} style={{
